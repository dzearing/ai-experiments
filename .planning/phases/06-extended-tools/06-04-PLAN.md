...
<task type="auto">
  <name>Task 2: Integrate TodoWriteDisplay and verify all Phase 6 tools</name>
  <files>
    apps/claude-code-web/client/src/components/ToolResultDisplay.tsx
  </files>
  <action>
...
5. Fix any remaining build or lint issues
  ...
  <verify>
    ...
    - All Phase 6 tools integrated: `grep -E "case\s*['\"]?(Write|Edit|Bash|TaskOutput|WebSearch|WebFetch|NotebookEdit|TodoWrite)['\"]?" apps/claude-code-web/client/src/components/ToolResultDisplay.tsx` returns all 8 tools
  </verify>
  <done>
    TodoWrite tool routes to TodoWriteDisplay in ToolResultDisplay; all 8 Phase 6 tools have visualization components verified via grep
  </done>
</task>
...
6. All Phase 6 tools verified via grep:
   ```bash
   grep -E "case\s*['\"]?(Write|Edit|Bash|TaskOutput|WebSearch|WebFetch|NotebookEdit|TodoWrite)['\"]?" apps/claude-code-web/client/src/components/ToolResultDisplay.tsx | wc -l
   ```
   Should return 8 (one case per tool)
</verification>
...
- All 8 Phase 6 extended tools are visualized (verified via grep pattern)
- Build and lint pass
</success_criteria>
