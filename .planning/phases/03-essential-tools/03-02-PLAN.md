...
<objective>
Create the Read tool visualization component with syntax highlighting and tool execution progress indicator.
...
Output: ToolResultDisplay router component that dispatches to tool-specific renderers, FileContentResult component that displays Read tool output using CodeBlock, and ToolExecutionIndicator for showing tool execution progress.
</objective>
...
<task type="auto">
  <name>Task 1: Create ToolExecutionIndicator component</name>
  <files>apps/claude-code-web/client/src/components/ToolExecutionIndicator.tsx, apps/claude-code-web/client/src/components/ToolExecutionIndicator.module.css</files>
  <action>
Create a component that displays a progress indicator while a tool is executing:
...
Structure:
1. Animated spinner or pulsing indicator (CSS animation)
2. Tool name displayed next to indicator: "Running {toolName}..."
3. Only renders when isExecuting is true
...
- .toolName - text styling
  ...
  <done>ToolExecutionIndicator displays animated progress indicator with tool name while executing</done>
</task>

<task type="auto">
  <name>Task 2: Create ToolResultDisplay router component</name>
...
Implementation:
1. If isExecuting is true, render ToolExecutionIndicator
2. Switch on toolName for completed tools:
...
Note on UI-06 (visual grouping): For Phase 3 MVP, tool results render inline within messages. Enhanced visual grouping with borders/containers deferred to Phase 6 polish pass. Current inline rendering is acceptable.
...
- .defaultResult - pre block styling with overflow scroll
  ...
  <verify>TypeScript compiles: `cd apps/claude-code-web/client && pnpm build`</verify>
  <done>ToolResultDisplay shows progress indicator when executing and routes to appropriate renderer when complete</done>
</task>

<task type="auto">
  <name>Task 3: Create FileContentResult component</name>
...
<task type="auto">
  <name>Task 4: Integrate ToolResultDisplay into ChatMessage tool output</name>
...
2. ToolExecutionIndicator shows animated spinner with tool name
3. ToolResultDisplay component exports correctly
4. FileContentResult renders with CodeBlock
5. Language detection works for common file types
6. No regressions in existing chat functionality
</verification>
