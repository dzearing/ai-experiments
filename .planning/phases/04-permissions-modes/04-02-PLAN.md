...
must_haves:
  truths:
    ...
    - "Denied permissions are tracked and shown in the message stream"
  artifacts:
    ...
    - path: "apps/claude-code-web/client/src/components/PermissionDeniedNotice.tsx"
      provides: "Denied permission inline notice"
      exports: ["PermissionDeniedNotice"]
...
<objective>
Implement client-side permission dialog components that display when the SDK requests tool approval, allowing users to approve, deny, or approve-always. Track and display denied permissions in the message stream.

Purpose: Give users control over tool execution by showing what Claude wants to do and letting them decide. Show denied permissions so users understand why Claude couldn't complete a task.
Output: PermissionDialog and AskUserDialog components, PermissionDeniedNotice for inline display, SSE event handling in useAgentStream, response POST to server.
</objective>
...
5. `DeniedPermission` interface:
   - toolName: string
   - input: Record<string, unknown>
   - reason: string
   - timestamp: number

6. Add these to an SSEEvent union type

7. Update `UseAgentStreamReturn` interface to add:
...
1. Add state for:
   ...
   - deniedPermissions: DeniedPermission[] (track denied permissions for the session)
...
5. Add clearDeniedPermissions function for session reset

6. Return new state and functions from the hook

7. Update startStream to accept permissionMode parameter and include in query string
  </action>
...
- Denied permissions tracked in state
  </done>
</task>

<task type="auto">
  <name>Task 2: Create PermissionDialog and PermissionDeniedNotice components</name>
  <files>
    apps/claude-code-web/client/src/components/PermissionDialog.tsx
    apps/claude-code-web/client/src/components/PermissionDialog.module.css
    apps/claude-code-web/client/src/components/PermissionDeniedNotice.tsx
...
**Create PermissionDeniedNotice.tsx:**

1. Import Text from @ui-kit/react
2. Import ShieldIcon or similar from @ui-kit/icons

3. Props interface:
   - toolName: string
   - reason: string

4. Render an inline notice showing:
   - Warning/shield icon
   - Text: "{toolName} was denied: {reason}"

5. Style with muted colors (not alarming, just informational)

**Create PermissionDeniedNotice.module.css:**

- .notice: flex row, align items center, gap, padding
- .icon: muted color
- .text: smaller font, muted color
- Use design tokens for colors (e.g., --color-body-textSoft20)
  </action>
  <verify>Build passes: `cd apps/claude-code-web/client && pnpm build`</verify>
  <done>
- PermissionDialog component renders tool name, formatted input, and approve/deny/always-allow buttons
- PermissionDeniedNotice component displays inline denied permission message
  </done>
</task>

<task type="auto">
  <name>Task 3: Create AskUserDialog and integrate all dialogs into ChatView</name>
  <files>
    apps/claude-code-web/client/src/components/AskUserDialog.tsx
    apps/claude-code-web/client/src/components/ChatView.tsx
  </files>
...
1. Import PermissionDialog, AskUserDialog, and PermissionDeniedNotice components
...
5. In the message stream rendering, after tool results or when appropriate, show denied permissions:
   - Map over deniedPermissions array and render PermissionDeniedNotice for each
   - Only show recent denials (within current conversation context)
   - Consider showing in the messages area or as a collapsible summary
  </action>
  <verify>
...
- Denied permissions displayed in message stream via PermissionDeniedNotice
  </done>
</task>
...
5. PermissionDeniedNotice component exports correctly
6. useAgentStream returns permission state and response functions
7. Denied permissions are tracked and displayed
</verification>
...
- Denied permissions tracked in state and displayed as inline notices in message stream
- ChatView integrates all dialogs and denied permission display
</success_criteria>
