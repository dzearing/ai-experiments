...
async function validateClaudeCodeState() {
  ...
  console.log(
    '❌ [ClaudeCodeProvider] Component cleanup running (should NOT appear during navigation)'
  );
  ...
  try {
    ...
    if (recentSSEEvents.length > 0) {
      ...
      recentSSEEvents.forEach((line) => {
        if (line.includes('CONNECTED')) {
          ...
          console.log(
            `📨 ${line.split('|')[0].trim()} - Greeting sent to ${activeConnections} connections`
          );
        }
      });
      console.log('');
    ...
    const hasConnectedDisconnectedPattern = recentEvents.some((line, index) => {
      if (line.includes('CLAUDE_SSE_CONNECTED')) {
        ...
        const disconnectCount = nextFewLines.filter((l) =>
          l.includes('CLAUDE_SSE_DISCONNECTED')
        ).length;
