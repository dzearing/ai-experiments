...
import { TopicService, type CreateTopicInput, type TopicMetadata } from './TopicService.js';
...
export interface ImportCallbacks {
  ...
  onComplete: (createdTopics: TopicMetadata[]) => void;
...
/**
 * Link to be created for a Topic
 */
interface TopicLinkPlan {
...
/**
 * Topic plan generated by Claude
 */
interface TopicPlan {
  name: string;
  description?: string;
  ...
  tags?: string[];
  /** File paths, URLs, or package references related to this topic */
  links?: TopicLinkPlan[];
 ...
 * Uses Claude to analyze sources and create Topics based on user instructions.
 */
export class ImportAgentService {
  ...
  async runImport(request: ImportRequest, callbacks: ImportCallbacks): Promise<void> {
    ...
    const createdTopics: TopicMetadata[] = [];
    let stepCounter = 0;
    ...
    try {
      ...
      callbacks.onStepStart({ id: analyzeStepId, label: 'Planning Topic structure...' });
      ...
      const topicCount = this.countTopicsInPlan(plan);
      callbacks.onStepComplete(analyzeStepId, `Planned ${topicCount} Topic${topicCount !== 1 ? 's' : ''} to create`);

      // Step 3: Create Topics
      const createStepId = nextStepId();
      callbacks.onStepStart({ id: createStepId, label: topicCount === 1 ? 'Creating a Topic...' : `Creating ${topicCount} Topics...` });

      await this.createTopicsFromPlan(
        plan,
        request.targetTopicId,
        request.userId,
        request.workspaceId,
      ...
      callbacks.onStepComplete(createStepId, createdTopics.length === 1 ? 'Created 1 Topic' : `Created ${createdTopics.length} Topics`);
      ...
      await chatService.updateSessionStatus(sessionId, 'completed', { topicsCreated: createdTopics.length });

      callbacks.onComplete(createdTopics);
    } catch (error) {
...
TASK: Analyze this package and create a Topic with rich metadata.
...
CRITICAL NAMING RULE: The "name" field MUST be the EXACT package name from package.json (the "name" field in package.json). Do NOT rename it to sometopic "friendly" like "Icon Library" - use the actual name like "@ui-kit/icons".
...
CRITICAL: Your response must be a single JSON object (not an array). Start with '{' and end with '}'. No explanations, no markdown.`;
   ...
   */
  private parsePackageResponse(response: string): TopicPlan {
    ...
    return JSON.parse(jsonStr) as TopicPlan;
  }
  ...
  private async analyzeWithDecomposition(
    ...
    const subTasks: SubTask<TopicPlan>[] = await Promise.all(
    ...
    const results = await orchestrator.run(
      {
        ...
        decompose: () => subTasks,
        aggregate: (outputs: TopicPlan[]) => outputs,
  ...
  private instructionsRequestMultiple(instructions: string): boolean {
    ...
    const multipleKeywords = [
      ...
      'everytopic', 'import all', 'add all', 'scan all',
  ...
  /**
   * Analyze source with Claude to generate a Topic plan
   */
  private async analyzeWithClaude(
    ...
    if (sourcePath && this.instructionsRequestMultiple(instructions)) {
      ...
      if (packages.length >= this.DECOMPOSITION_THRESHOLD) {
        ...
        // Fall through to regular analysis if decomposition returned notopic
...
TASK: Analyze this source content and create Topics based on user instructions.
...
[
  {
    ...
    "children": [/* nested Topics - ONLY if user explicitly asks for children/sub-items */]
  }
]
...
1. **FOLLOW USER INSTRUCTIONS LITERALLY**
   - If user says "add the icons package", create ONLY ONE Topic for that specific package
   - If user says "add all packages", then add multiple Topics
...
3. **SCOPE - Only create what was asked**
   ...
   - If user asks for "all packages" or "everytopic", then return multiple items
   - When in doubt, create fewer Topics rather than more
...
CRITICAL: Your response must start with '[' and end with ']'. No explanations, no markdown. Just the raw JSON array.`;
    ...
    try {
            ...
            for (const block of msgContent) {
              if (block.type === 'text') {
                ...
                if (now - lastUpdateTime > updateInterval) {
                  ...
                  const topicCount = nameMatches ? nameMatches.length : 0;
                  if (topicCount > 0) {
                    callbacks.onStepUpdate(stepId, {
                      detail: `Building plan... (${topicCount} Topic${topicCount !== 1 ? 's' : ''} found)`
                    });
                  }
                  lastUpdateTime = now;
                }
      ...
      const plan: TopicPlan[] = JSON.parse(jsonStr);
      return plan;
    } catch (error) {
  ...
  /**
   * Count total Topics in a plan (including nested)
   */
  private countTopicsInPlan(plans: TopicPlan[]): number {
    let count = 0;
    for (const plan of plans) {
      count++;
      if (plan.children) {
        count += this.countTopicsInPlan(plan.children);
      }
    }
    return count;
  }
  ...
  /**
   * Create Topics from a plan recursively
   */
  private async createTopicsFromPlan(
    plans: TopicPlan[],
    parentId: string,
    userId: string,
    ...
    for (const plan of plans) {
      ...
      // Convert links from plan format to CreateTopicInput format
      ...
      // Map icon string to valid TopicIcon type
      ...
      // Map color string to valid TopicColor type
      ...
      const created = await this.topicService.createTopic(userId, input);
