...
/**
 * Service for the Plan Agent to edit Yjs documents (Implementation Plan).
 *
 ...
 */
export class PlanAgentYjsClient {
  ...
  private static readonly AGENT_USER: AgentUser = {
    name: 'Plan Agent',
    color: '#10b981', // Green (distinct from Idea Agent's purple)
  };
  ...
  async connect(roomName: string): Promise<AgentSession> {
    ...
    console.log(`[PlanAgentYjsClient] Connected to room "${roomName}" with clientId ${clientId}`);
  ...
  disconnect(roomName: string): void {
    ...
    console.log(`[PlanAgentYjsClient] Disconnected from room "${roomName}"`);
  }
  ...
  async replaceContent(roomName: string, content: string): Promise<void> {
    ...
    console.log(`[PlanAgentYjsClient] Replaced content in room "${roomName}" (${content.length} chars)`);
  }
  ...
  async streamContent(
    ...
    console.log(`[PlanAgentYjsClient] Starting to stream ${content.length} chars to room "${roomName}"`);

    const batchSize = PlanAgentYjsClient.STREAM_BATCH_SIZE;
    const batchDelay = PlanAgentYjsClient.STREAM_BATCH_DELAY;

    // Stream in batches to prevent overwhelming CodeMirror's view update system
    const cursorInterval = PlanAgentYjsClient.CURSOR_UPDATE_INTERVAL;
    let batchCount = 0;
    ...
    for (let i = 0; i < content.length; i += batchSize) {
      ...
      if (charsInserted % 200 < batchSize) {
        console.log(`[PlanAgentYjsClient] Streamed ${charsInserted}/${content.length} chars`);
      }
    ...
    console.log(`[PlanAgentYjsClient] Streamed ${content.length} chars to room "${roomName}"`);
  }
    ...
    try {
      switch (edit.action) {
        case 'replace': {
          ...
          console.log(`[PlanAgentYjsClient] Replace: found range ${replaceStart}-${replaceEnd} (hint: ${edit.start})`);
        ...
        case 'insert': {
          ...
          console.log(`[PlanAgentYjsClient] Insert: position ${insertPosition} (after "${edit.afterText.slice(0, 30)}...")`);
        ...
        case 'delete': {
          ...
          console.log(`[PlanAgentYjsClient] Delete: found range ${deleteStart}-${deleteEnd} (hint: ${edit.start})`);
  ...
  async applyEdits(roomName: string, edits: DocumentEdit[]): Promise<EditResult[]> {
    ...
    console.log(`[PlanAgentYjsClient] Applied ${edits.length} edits to room "${roomName}"`);
    return results;
  }
