...
interface UseTopicIdeasReturn {
  ...
  createIdea: (input: Omit<CreateIdeaInput, 'topicIds'>) => Promise<Idea | null>;
...
/**
 * Hook for managing ideas scoped to a specific Topic.
 * Keeps Topic ideas separate from global IdeasContext.
 */
export function useTopicIdeas(topicId: string, workspaceId?: string): UseTopicIdeasReturn {
  const { user } = useAuth();
  ...
  const fetchIdeas = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/ideas/by-topic/${topicId}`, {
        headers: {
          'x-user-id': user.id,
        },
      });
  ...
  // Fetch on mount and when topicId changes
  useEffect(() => {
    fetchIdeas();
  }, [fetchIdeas]);

  // Create a new idea linked to this Topic
  const createIdea = useCallback(async (input: Omit<CreateIdeaInput, 'topicIds'>): Promise<Idea | null> => {
