...
import type { TopicLink, TopicLinkType } from '../../types/topic';
import styles from './TopicLinks.module.css';

export interface TopicLinksRef {
  startAdd: () => void;
}

interface TopicLinksProps {
  links: TopicLink[];
  onAdd: (link: Omit<TopicLink, 'id' | 'createdAt'>) => Promise<void>;
  onUpdate: (linkId: string, updates: Partial<TopicLink>) => Promise<void>;
  onRemove: (linkId: string) => Promise<void>;
  disabled?: boolean;
  hideAddButton?: boolean;
}

const LINK_TYPE_ICONS: Record<TopicLinkType, ReactNode> = {
  file: <FileIcon />,
  url: <LinkIcon />,
  github: <CodeIcon />,
  package: <PackageIcon />,
...
const LINK_TYPE_LABELS: Record<TopicLinkType, string> = {
  file: 'Local File',
  url: 'URL',
  github: 'GitHub',
  package: 'Package',
...
export const TopicLinks = forwardRef<TopicLinksRef, TopicLinksProps>(function TopicLinks(
  ...
  const handleOpenFile = useCallback(async (path: string) => {
    if (!user) return;
    try {
      await fetch(`${API_URL}/api/topics/open-path`, {
        method: 'POST',
        headers: {
  ...
  const handleStartEdit = useCallback((link: TopicLink) => {
    setIsAdding(false);
        ...
        <div className={styles.form}>
          <div className={styles.formField}>
            ...
            <div className={styles.typeSelector}>
              {(Object.keys(LINK_TYPE_LABELS) as TopicLinkType[]).map((type) => (
                <button
                  key={type}
                  type="button"
