...
interface ImportDialogProps {
  ...
  onClose: () => void;
  /** The topic we're importing into */
  targetTopic: TopicMetadata;
}

type WizardStep = 'source' | 'config' | 'progress';

export function ImportDialog({ open, onClose, targetTopic }: ImportDialogProps) {
  const { fetchTopicsGraph } = useTopics();
  const {
    startImport,
    cancelImport,
    steps: progressSteps,
  ...
  }, [sourceType, gitUrl, localPath, instructions, targetTopic, startImport, resetImport]);
  ...
  // Refresh topics list when import completes
  useEffect(() => {
    if (isComplete) {
      fetchTopicsGraph(targetTopic.workspaceId);
    }
  }, [isComplete, fetchTopicsGraph, targetTopic.workspaceId]);

  // Render source selection step
  const renderSourceStep = () => (
    <div className={styles.sourceStep}>
      <Text size="sm" color="soft" className={styles.stepDescription}>
        Choose how you want to import content into "{targetTopic.name}"
      </Text>
    ...
    <div className={styles.configStep}>
      ...
      <div className={styles.field}>
        ...
        <Textarea
          ...
          placeholder={`Describe how you want to organize the imported content as Topics. For example:\n\n"Create a topic called 'Packages', and under it create a topic for each package in the monorepo. For each package, add description from its README."`}
          rows={6}
        />
        <Text size="xs" color="soft" className={styles.hint}>
          Describe the Topic structure you want to create from this source
        </Text>
      </div>
    </div>
  );
  ...
  return (
    <Dialog
      ...
      title={`Import into "${targetTopic.name}"`}
      size="md"
      footer={renderFooter()}
    >
