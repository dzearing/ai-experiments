...
import type { ThingIcon, ThingColor } from '../../types/thing';
import styles from './ThingStylePicker.module.css';

interface ThingStylePickerProps {
  icon?: ThingIcon | null;
  color?: ThingColor | null;
  onIconChange: (icon: ThingIcon | null) => void;
  onColorChange: (color: ThingColor | null) => void;
}

// Map ThingIcon to React components
...
// Map ThingColor to surface preset names
const COLOR_TO_PRESET: Record<ThingColor, string> = {
  default: 'steel',
  blue: 'sky',
  green: 'emerald',
  purple: 'grape',
...
const COLOR_NAMES = Object.keys(COLOR_TO_PRESET) as ThingColor[];

// Get surface colors for a ThingColor
function getSurfaceColors(color: ThingColor, mode: 'light' | 'dark' = 'light'): SurfaceColors | null {
  const presetName = COLOR_TO_PRESET[color];
  return generateSurfaceFromPreset(presetName, mode);
}

export function ThingStylePicker({
  icon,
  color,
  onIconChange,
  onColorChange,
}: ThingStylePickerProps) {
  ...
  const handleIconSelect = (iconName: ThingIcon) => {
    onIconChange(iconName === icon ? null : iconName);
  };

  const handleColorSelect = (colorName: ThingColor) => {
