...
/**
 * Topics CRUD E2E Tests
 *
 * Tests creating, renaming, and deleting topics in the tree view.
...
const TEST_USER_NAME = 'Topics E2E Test';
const TEST_USER = {
  id: generateUserId(TEST_USER_NAME),
  name: TEST_USER_NAME,
  avatarUrl: 'https://api.dicebear.com/7.x/initials/svg?seed=Topics',
};
...
async function createWorkspaceViaAPI(
  ...
  const response = await request.post(`${API_BASE}/workspaces`, {
    data: { name, description: 'Test workspace for topics' },
...
async function createTopicViaAPI(
  ...
  const response = await request.post(`${API_BASE}/topics`, {
    data: {
      type: 'item',
      ...data,
    },
 ...
 */
async function deleteTopicViaAPI(
  request: APIRequestContext,
  topicId: string,
  userId: string
): Promise<void> {
  const response = await request.delete(`${API_BASE}/topics/${topicId}`, {
    headers: { 'x-user-id': userId },
  });
  expect(response.ok()).toBeTruthy();
}
...
async function getTopicsViaAPI(
  ...
  const url = workspaceId
    ? `${API_BASE}/topics?workspaceId=${workspaceId}`
    : `${API_BASE}/topics`;
...
/**
 * Navigate to the Topics page and wait for it to load
 */
async function navigateToTopics(page: Page) {
  await page.goto('/topics');
  // Wait for the Topics heading and tree container to be visible
  await expect(page.locator('h2:has-text("Topics")')).toBeVisible({ timeout: 10000 });
  // Wait for loading to complete (no "Loading" indicator)
  await expect(page.locator('text=Loading')).not.toBeVisible({ timeout: 10000 });
}

/**
 * Click the "Create a Topic" button to start inline editing
 */
async function clickCreateButton(page: Page) {
  await page.click('button:has-text("Create a Topic")');
  // Wait for the input to appear
  await expect(page.locator('input[placeholder*="New topic"]')).toBeVisible();
}
...
async function getTreeItemsInOrder(page: Page): Promise<string[]> {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (name && !name.includes('New topic') && !name.includes('New child')) {
      names.push(name);
    }
  }
  return names;
 ...
 */
async function selectTopic(page: Page, topicName: string) {
  await page.click(`[role="treeitem"]:has-text("${topicName}")`);
...
test.describe('Topics CRUD Operations', () => {
  ...
  test.beforeEach(async ({ page, request }) => {
    ...
    // Clean up any existing topics from previous test runs FIRST
    // Delete root-level topics only (cascade delete handles children)
    try {
      const existingTopics = await getTopicsViaAPI(request, TEST_USER.id);
      // Filter to only root topics (no parents) - cascade delete will handle children
      const rootTopics = existingTopics.filter(t => !t.parentIds || t.parentIds.length === 0);
      for (const topic of rootTopics) {
        try {
          await deleteTopicViaAPI(request, topic.id, TEST_USER.id);
        } catch {
          // Ignore cleanup errors
        }
      }
      // Double-check: if any orphaned children remain, delete them too
      const remaining = await getTopicsViaAPI(request, TEST_USER.id);
      ...
      // Ignore if no topics exist
    }

    // Create a test workspace
    const workspaceName = `Topics Test Workspace ${Date.now()}`;
    testWorkspace = await createWorkspaceViaAPI(request, workspaceName, TEST_USER.id);
  });

  test.afterEach(async ({ request }) => {
    // Cleanup: delete all topics first, then workspace
    if (testWorkspace?.id) {
      try {
        const topics = await getTopicsViaAPI(request, TEST_USER.id);
  ...
  test('creates topics in correct sequential order (1, 2, 3)', async ({ page }) => {
  ...
  test('creates child nodes using Tab to indent', async ({ page }) => {
    ...
    await clickCreateButton(page);
    let input = page.locator('input[placeholder*="New topic"]');
  ...
  test('creates sibling after selected item with Ctrl+Enter', async ({ page, request }) => {
    ...
    const topic = await createTopicViaAPI(request, { name: 'Existing Item' }, TEST_USER.id);
    ...
    // Select the item
    await selectTopic(page, 'Existing Item');
  ...
  test('renames topic with F2 key', async ({ page, request }) => {
    // Create a topic via API
    const topic = await createTopicViaAPI(request, { name: 'Original Name' }, TEST_USER.id);
    ...
    // Wait for the topic to appear and select it
    await expect(page.locator('[role="treeitem"]:has-text("Original Name")')).toBeVisible();
    await selectTopic(page, 'Original Name');
  ...
  test('cancels rename with Escape key', async ({ page, request }) => {
    // Create a topic via API
    const topic = await createTopicViaAPI(request, { name: 'Keep This Name' }, TEST_USER.id);
    ...
    await selectTopic(page, 'Keep This Name');

    // Press F2 to start renaming
    const treeContainer = page.locator('[role="tree"]').first();
    await treeContainer.press('F2');

    // Type sometopic in the rename input
    const renameInput = page.locator('[role="treeitem"] input');
    await expect(renameInput).toBeVisible();
  ...
  test('deletes topic with Delete key showing confirmation dialog', async ({ page, request }) => {
    // Create a topic via API
    const topic = await createTopicViaAPI(request, { name: 'To Be Deleted' }, TEST_USER.id);
    ...
    await selectTopic(page, 'To Be Deleted');
    ...
    await expect(dialog.locator('text=Delete Topic')).toBeVisible();
  ...
  test('deletes topic immediately with Ctrl+Delete (no dialog)', async ({ page, request }) => {
    // Create a topic via API
    const topic = await createTopicViaAPI(request, { name: 'Quick Delete' }, TEST_USER.id);
    ...
    await selectTopic(page, 'Quick Delete');
  ...
  test('selects next item after deletion', async ({ page, request }) => {
    ...
    const topic1 = await createTopicViaAPI(request, { name: 'First' }, TEST_USER.id);
    const topic2 = await createTopicViaAPI(request, { name: 'Second' }, TEST_USER.id);
    const topic3 = await createTopicViaAPI(request, { name: 'Third' }, TEST_USER.id);
    ...
    // Cleanup
    await deleteTopicViaAPI(request, topic1.id, TEST_USER.id);
    await deleteTopicViaAPI(request, topic3.id, TEST_USER.id);
  });

  test('keyboard navigation works after operations', async ({ page, request }) => {
    // Create topics via API
    const topic1 = await createTopicViaAPI(request, { name: 'Alpha' }, TEST_USER.id);
    const topic2 = await createTopicViaAPI(request, { name: 'Beta' }, TEST_USER.id);
    ...
    // Cleanup
    await deleteTopicViaAPI(request, topic1.id, TEST_USER.id);
    await deleteTopicViaAPI(request, topic2.id, TEST_USER.id);
  });
});

test.describe('Topics Hierarchy Operations', () => {
  test.beforeEach(async ({ page, request }) => {
    await signIn(page);

    // Clean up any existing topics from previous test runs
    try {
      const existingTopics = await getTopicsViaAPI(request, TEST_USER.id);
