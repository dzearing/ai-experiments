...
async function setupClaudeCodeSession(page: any) {
  ...
  await page.fill('textarea[placeholder="Enter project description"]', 'Project for testing feedback feature');
...
test.describe('Feedback Feature', () => {
  ...
  test('should capture screenshot when feedback is initiated', async ({ page }) => {
    ...
    const screenshotPromise = page.waitForRequest(req => 
      req.url().includes('/api/feedback/screenshot') && req.method() === 'POST'
    );
  ...
  test('should show feedback link on tool executions', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="tool-status"]:has-text("Complete")', { timeout: 30000 });
  ...
  test('should include correct message context in feedback', async ({ page }) => {
    ...
    const userMsgInFeedback = feedbackData.messages.find((m: any) => 
      m.content === 'Test message for feedback'
    );
  ...
  test('should save feedback report with server logs', async ({ page, request }) => {
    ...
    const response = await request.post('http://localhost:3000/api/feedback/submit', {
      data: {
        ...
        messages: [
          { id: 'msg-123', role: 'user', content: 'Test message' }
        ],
        mode: 'default',
        isConnected: true,
    ...
    const content = JSON.parse(
      fs.readFileSync(path.join(reportsDir, feedbackFile!), 'utf8')
    );
