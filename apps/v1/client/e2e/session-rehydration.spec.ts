...
test.describe('Session Rehydration', () => {
  test('should preserve chat messages when navigating away and back to a session', async ({ page, testWorkspace }) => {
  ...
  test('should handle multiple session switches correctly', async ({ page, testWorkspace }) => {
    ...
    const repo2Button = page.locator('button').filter({ hasText: /hello-world-\d+/i }).nth(1);
    if (await repo2Button.count() > 0) {
      ...
      const restoredContent = await page.locator('[data-testid="message-bubble"]').first().textContent();
      expect(restoredContent).toBeTruthy();
