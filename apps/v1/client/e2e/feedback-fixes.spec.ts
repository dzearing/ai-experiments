...
test.describe('Feedback Dialog Fixes', () => {
  test('feedback dialog should be centered on window and use portal rendering', async ({
    page,
  }) => {
    ...
    const dialog = await page
      .locator('body > div')
      .filter({ has: page.locator('h2:has-text("Leave feedback")') });
  ...
  test('suggested responses should appear for plan mode questions', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const lastMessage = Array.from(messages)
        .reverse()
        .find(
          (m) => m.querySelector('.text-green-500') // Assistant messages have green avatar
        );
  ...
  test('mode should switch from plan to execution when user approves', async ({ page }) => {
    ...
    await expect(
      page.locator('button[aria-label="Claude mode"]:has-text("Default")')
    ).toBeVisible();
  });
