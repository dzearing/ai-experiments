...
test.describe('Claude Code Mount/Unmount Issue', () => {
  test('should capture console logs when navigating to Claude Code', async ({
    page,
    testWorkspace,
  }) => {
    ...
    const mountPatterns = consoleLogs.filter(
      (log) =>
    ...
    const hasMessageNotFound = consoleLogs.some((log) =>
      log.includes('Message not found for chunk')
    );
