...
test.describe('Plan Mode Restrictions', () => {
  ...
  test('should not allow write operations in plan mode', async ({ page, testWorkspace }) => {
    ...
    const writeTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Write file")')
      .count();
    const editTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Edit file")')
      .count();
    const bashTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Run command")')
      .count();
    ...
    const readTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Read file")')
      .count();
    const searchTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Search files")')
      .count();
    const findTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Find files")')
      .count();
    const listTools = await page
      .locator('[data-testid="tool-execution"]:has-text("List directory")')
      .count();
  ...
  test('should show exit plan mode tool when appropriate', async ({ page, testWorkspace }) => {
    ...
    await page.waitForSelector('[data-testid="tool-execution"]:has-text("Exit plan mode")', {
      timeout: 30000,
    });

    // Verify the exit plan mode tool was executed
    const exitPlanTools = await page
      .locator('[data-testid="tool-execution"]:has-text("Exit plan mode")')
      .count();
