...
export function LoadingSpinner({
  ...
  const spinnerElement = (
    <div className={`flex flex-col items-center justify-center ${spinnerColorClass} ${className}`}>
      <div
        className={`animate-spin rounded-full ${sizeClasses[size].spinner} border-current border-t-transparent`}
      />
      {text && <p className={`mt-2 ${styles.mutedText} ${sizeClasses[size].textSize}`}>{text}</p>}
    </div>
  );
