...
export const TodoList = memo(function TodoList({ todos, onDismiss }: TodoListProps) {
  ...
  const todosByStatus = hierarchicalTodos.reduce(
    (acc, todo) => {
      if (!acc[todo.status]) {
        acc[todo.status] = [];
      }
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'in_progress':
        return <Spinner size="small" className="text-blue-500 flex-shrink-0" />;
      case 'pending':
        return (
          <svg
    ...
    return (
      <React.Fragment key={todo.id}>
        ...
        >
          ...
          <p
            className={`text-sm ${styles.textColor} ${isCompleted ? 'line-through opacity-60' : ''} truncate flex-1`}
          >
            {todo.content}
          </p>
        </div>
