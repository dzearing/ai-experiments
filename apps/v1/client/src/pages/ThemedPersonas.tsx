    ...
    <div>
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className={`mt-1 ${styles.mutedText}`}>Manage your AI agents and their assignments.</p>
        </div>
        {personas.length > 0 && (
          <Button as={Link} to="/agents/new/1" variant="primary">
            Create new agent
          </Button>
        )}
      </div>
      ...
      {/* Personas Table */}
      <div
        className={`${styles.cardBg} ${styles.cardBorder} border ${styles.borderRadius} ${styles.cardShadow} overflow-hidden`}
      >
        <table className="min-w-full">
          <thead className={`${styles.contentBg} ${styles.contentBorder} border-b`}>
            <tr>
              <th
                scope="col"
                className={`px-6 py-3 text-left text-sm font-semibold ${styles.headingColor} w-64`}
              >
                Agent
              </th>
              <th
                scope="col"
                className={`px-6 py-3 text-left text-sm font-semibold ${styles.headingColor}`}
              >
                Personality
              </th>
              <th
                scope="col"
                className={`px-6 py-3 text-left text-sm font-semibold ${styles.headingColor} w-32`}
              >
                Status
              </th>
              <th
                scope="col"
                className={`px-6 py-3 text-left text-sm font-semibold ${styles.headingColor} w-28`}
              >
                Actions
              </th>
            </tr>
            ...
            {personas.length === 0 ? (
              <tr>
                <td colSpan={4} className="px-6 py-12 text-center">
                  ...
                  <h3 className={`mt-4 text-lg font-medium ${styles.headingColor}`}>
                    No agents yet
                  </h3>
                  ...
                  <tr key={persona.id}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div
                            className={`text-sm font-medium ${styles.headingColor} whitespace-nowrap`}
                          >
                            {persona.name}
                          </div>
