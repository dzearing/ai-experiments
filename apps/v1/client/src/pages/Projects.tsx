      ...
      {isLoadingWorkspace ? (
        ...
        <div className="grid gap-4">
          {projects.map((project) => {
            const primaryRepo =
              project.repositories?.find((r) => r.isPrimary) || project.repositories?.[0];
                        ...
                        {project.repositories.map((repo, index) => {
                          ...
                          if (repo.url.startsWith('local://')) {
                            return (
                              <div
                                key={index}
                                className={`inline-flex items-center gap-2 px-3 py-1.5 text-sm ${styles.contentBg} ${styles.contentBorder} border ${styles.borderRadius}`}
                              >
                                {getRepoTypeIcon(repo.type)}
                                <span className={styles.mutedText}>Local development</span>
                                {repo.isPrimary && (
                                  <span
                                    className={`px-1.5 py-0.5 text-xs ${styles.primaryButton} ${styles.primaryButtonText} rounded`}
                                  >
                                    Primary
                                  </span>
                                )}
        ...
        onConfirm={async (_, removeFolder) => {
          try {
            ...
            const response = await fetch(
              `http://localhost:3000/api/projects/${encodeURIComponent(projectName)}/${endpoint}?workspacePath=${encodeURIComponent(workspacePath || '')}`,
              {
                method: 'POST',
              }
            );
