...
export function ProjectDetail() {
  ...
  const project = workspaceProjects.find(
    (p: WorkspaceProject) => p.name === appProject?.name || p.path?.includes(projectId || '')
  );
  ...
  useEffect(() => {
    ...
    const unsubscribers = project.repos.map((repo: WorkspaceRepo) => {
      return subscribe(
        'repo-status',
        `${project.path}/${repo.name}-${repo.number}`,
        (status: RepoStatus) => {
  ...
  if (!project) {
    return (
      <div className={`text-center py-12 ${styles.textColor}`}>
        ...
        <Link
          to="/projects"
          className={`${styles.linkText || 'text-blue-600 dark:text-blue-400'} hover:underline mt-4 inline-block`}
        >
          Back to projects
        </Link>
      </div>
  ...
  const handleRebase = async (repo: WorkspaceRepo) => {
    ...
    try {
      const response = await fetch(
        apiUrl(
          `/api/repos/${encodeURIComponent(project.path || '')}/${repo.name}-${repo.number}/rebase`
        ),
        {
          method: 'POST',
        }
  ...
  const handleReset = async (repo: WorkspaceRepo, stashChanges: boolean) => {
    ...
    try {
      const response = await fetch(
        apiUrl(
          `/api/repos/${encodeURIComponent(project.path || '')}/${repo.name}-${repo.number}/reset`
        ),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        apiUrl(
          `/api/repos/${encodeURIComponent(project.path || '')}/${repoToDelete.name}-${repoToDelete.number}`
        ),
        {
          method: 'DELETE',
        }
  ...
  const getStatusBadge = (status: RepoStatus | undefined) => {
    ...
    if (status.error) {
      return (
        <span
          className={`px-2 py-1 text-xs rounded bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400`}
        >
          Error
        </span>
      );
    ...
    const textClass = status.isDirty ? styles.warningText : styles.successText;
  ...
  const handleClone = async () => {
    ...
    try {
      ...
      const response = await fetch(
        apiUrl(`/api/repos/${encodeURIComponent(project.path || '')}/clone`),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      ...
      {!isConnected && (
        <div
          className={`mb-4 p-3 rounded ${styles.warningBg || 'bg-yellow-100 dark:bg-yellow-900/20'} ${styles.warningText}`}
        >
      ...
      {/* Project info */}
      <div
        className={`mb-6 p-6 ${styles.cardBg} ${styles.cardBorder} border ${styles.borderRadius}`}
      >
      ...
      <div>
        ...
        {!project.repos || project.repos.length === 0 ? (
          <div
            className={`${styles.cardBg} ${styles.cardBorder} border ${styles.borderRadius} p-8 text-center`}
          >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {(() => {
                          ...
                          console.log(
                            `Checking badge for ${repoKey}: ${hasReservation}`,
                            'repo:',
                            repo,
                            'map:',
                            Array.from(repoReservations.keys())
                          );
                          return (
                            hasReservation && (
                              <span
                                className={`px-2 py-1 text-xs font-medium rounded bg-blue-100 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400`}
                              >
                          ...
                          {status.isDirty && (
                            <div className={`text-sm ${styles.mutedText} mb-2`}>
                              ...
                              {status.changes.modified > 0 &&
                                ` ${status.changes.modified} modified`}
                              ...
                              {status.changes.untracked > 0 &&
                                ` ${status.changes.untracked} untracked`}
                            </div>
                          )}
                      ...
                      {repo.activeWorkItem && (
                        <div className={`mt-3 text-sm ${styles.mutedText}`}>
                          Working on:{' '}
                          <span className={styles.textColor}>{repo.activeWorkItem}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2 ml-4">
                      ...
                      <Button
                        ...
                        data-repo-name={`${repo.name}-${repo.number}`}
                        onClick={() =>
                          navigate(`/projects/${projectId}/claude-code/${repo.name}-${repo.number}`)
                        }
                      >
      ...
      {showCloneDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
          <div
            className={`${styles.cardBg} ${styles.cardBorder} border ${styles.borderRadius} ${styles.cardShadow} p-6 max-w-lg w-full`}
          >
            <h3 className={`text-lg font-semibold ${styles.headingColor} mb-4`}>
