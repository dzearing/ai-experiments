    ...
    <div className='p-8'>
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <div
            key={stat.title}
            className={`overflow-hidden ${styles.cardShadow} ${styles.borderRadius} ${styles.cardBg} ${styles.cardBorder} border`}
          >
            <div className="p-5">
              <div className="flex items-center">
                <div className="flex-shrink-0">
                  <svg
                    className={`h-6 w-6 ${styles.mutedText}`}
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                  >
                ...
                <div className="ml-5 w-0 flex-1">
                  <dl>
                    <dt className={`text-sm font-medium ${styles.mutedText} truncate`}>
                      {stat.title}
                    </dt>
                    <dd className="flex items-baseline">
                      <div className={`text-2xl font-semibold ${styles.headingColor}`}>
                        {stat.value}
                      </div>
                    </dd>
                  </dl>
            ...
            <div className={`px-5 py-3 ${styles.contentBorder} border-t`}>
              <div className="text-sm">
                <Link
                  to={stat.link}
                  className={`font-medium ${styles.textColor} hover:opacity-80 transition-opacity`}
                >
                  View all →
                </Link>
              </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {quickActions.map((action) => (
            ...
            >
              <div className="flex-shrink-0">
                <svg
                  className={`h-10 w-10 ${styles.mutedText}`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
