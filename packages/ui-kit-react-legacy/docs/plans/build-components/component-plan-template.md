...
<!-- 
CRITICAL GUIDANCE FOR CREATING MOCKUPS:

1. UI-KIT SETUP (REQUIRED):
   a) Link to the built CSS file:
      <link rel="stylesheet" href="assets/styles.css">
   
   b) Set the base path for themes BEFORE loading scripts:
      <script>
          window.__uiKitBasePath = 'assets/';
      </script>
   
   c) Include the theme scripts (ES modules):
      <script type="module" src="assets/theme.js"></script>
      <script type="module" src="assets/theme-switcher.js"></script>
      
      - theme.js: Core theme engine (auto-initializes, prevents FOUC, provides __uiKitTheme global API)
      - theme-switcher.js: Adds theme/mode controls to the top-right of the page
      - Both are auto-updating when you rebuild ui-kit (no need to copy/paste code)

2. TOKEN USAGE:
   - Use ONLY tokens from the ui-kit design system
   - Reference docs/guides/TOKEN_CHEATSHEET.md for available tokens
   - Common token mappings:
     * Spacing: --spacing-small20 (4px), --spacing-small10 (8px), --spacing (16px), --spacing-large10 (24px)
     * Colors: --color-body-text, --color-panel-background, --color-body-border
     * Typography: --font-size-small10, --font-weight-medium, --line-height
     * Radius: --radius-small10 (4px), --radius (6px), --radius-large10 (8px), 50% for circles
     * Shadows: --shadow-soft10, --shadow-card, --shadow-hard10

3. MISSING TOKENS:
   - If a needed token doesn't exist, document it in docs/plans/MISSING_TOKEN_REQUESTS.md
   - Describe: scenario, needed token, expected value, suggested name
   - For component-specific styles (e.g., gradients), define them in a :root block

4. HTML/CSS BEST PRACTICES:
   - Use semantic HTML and proper CSS layout (flexbox/grid)
