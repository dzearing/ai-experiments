...
```tsx
// Convenient but relies on bundler optimization
import { SaveIcon, EditIcon, DeleteIcon } from '@ui-kit/icons';
```

> **Note:** Barrel imports work well with modern bundlers (Vite, webpack 5, esbuild) that support tree-shaking. For maximum control, use direct imports.
...
SVG files are located at `src/svgs/{category}/{name}.svg`. Copy the content directly:
...
1. Drop an SVG file into `src/svgs/{category}/`
2. Run `pnpm build`
3. The icon is automatically available in all formats
