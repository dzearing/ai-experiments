...
--duration-modal-in: var(--duration-normal);
--duration-modal-out: var(--duration-fast);
...
<Source
  ...
  code={`
.modal {
  animation: slide-in-up var(--duration-modal-in) var(--easing-enter);
}

.modal.closing {
  animation: fade-out var(--duration-modal-out) var(--easing-exit);
}
  `}
/>
...
### React Native
- Map duration tokens to Animated API
- Use native driver for transforms and opacity
- Consider gesture-driven animations

### Desktop (Electron)
- Match native OS animation speeds
- Respect system-wide animation preferences
- Use subtle animations to feel native