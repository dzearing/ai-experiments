...
describe('dynamicSurface', () => {
  describe('generateSurfaceColors', () => {
    ...
    it('caps saturation at maximum values', () => {
      ...
      // Extract saturation from background
      const satMatch = colors.bg.match(/hsl\(\d+,\s*([\d.]+)%/);
      const saturation = satMatch ? parseFloat(satMatch[1]) : 100;

      expect(saturation).toBeLessThanOrEqual(70); // Max for light mode
    });
  });
  ...
  describe('generateSurfaceFromPreset', () => {
    ...
    it('generates correct hue for coral preset', () => {
      ...
      expect(hue).toBe(16); // coral hue is 16
    });
  });
  ...
  describe('SURFACE_PRESETS', () => {
    ...
    it('all presets have valid hue values (0-360)', () => {
      Object.entries(SURFACE_PRESETS).forEach(([name, [hue]]) => {
        expect(hue, `${name} hue`).toBeGreaterThanOrEqual(0);
        expect(hue, `${name} hue`).toBeLessThanOrEqual(360);
      });
    });

    it('all presets have valid saturation values (0-100)', () => {
      Object.entries(SURFACE_PRESETS).forEach(([name, [, saturation]]) => {
        expect(saturation, `${name} saturation`).toBeGreaterThanOrEqual(0);
        expect(saturation, `${name} saturation`).toBeLessThanOrEqual(100);
      });
    });
  ...
  describe('injectSurfaceStyles', () => {
    ...
    it('does nothing when document is undefined', () => {
      // This test verifies the function handles missing DOM gracefully
      // In Node.js environment, this tests the early return
      // In browser environment, this still works because we test actual injection below
    ...
    it.skipIf(!hasDom)('replaces existing style element', () => {
      injectSurfaceStyles();
      injectSurfaceStyles();
    ...
    it.skipIf(!hasDom)('uses custom style ID', () => {
    ...
    it.skipIf(!hasDom)('includes all preset classes in injected CSS', () => {
      injectSurfaceStyles();
  ...
  describe('getRandomSurfaceName', () => {
    ...
    it('returns different values over multiple calls (probabilistic)', () => {
      ...
      // Call many times to increase probability of different values
      for (let i = 0; i < 100; i++) {
        names.add(getRandomSurfaceName());
      }

      // Should have gotten at least a few different names
